...
`);this.downloadFile(e,"sitemap.csv","text/csv")}catch{throw new Error("CSV export failed")}},300));zt(this,"exportJSON",$e(async()=>{try{const e=JSON.stringify(this.options.urls,null,2);this.downloadFile(e,"sitemap.json","application/json")}catch{throw new Error("JSON export failed")}},300));this.container=e,this.options={urls:n.urls||[],path:n.path||window.location.pathname,...n},this.listeners=new Map,this.render(),this.setupEventListeners()}render(){const e=this.options.path==="/results";this.container.innerHTML=`
  ...
  `;const e=t.querySelector("#sitemapUrlForm"),n=t.querySelector("#sitemapUrlInput"),r=t.querySelector("#sitemapUploadForm");return e.addEventListener("submit",async i=>{i.preventDefault();const a=n.value.trim();if(!a){pt("Please enter a sitemap URL");return}if(!Hs(a)){pt("Invalid URL: Must start with http:// or https:// and be well-formed");return}Y(10);try{const s=await fetch("/sitemap/url",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a})});if(!s.ok){const c=await s.json();throw new Error(c.error||"Failed to fetch sitemap")}const{id:o,message:u,status:l}=await s.json();Y(50),window.history.pushState({},"",`/results?id=${o}`),_e("/results"),de(`${u} (${l.succeeded.length}/${l.attempted.length} sitemaps succeeded)`)}catch(s){pt(s.message)}finally{nt()}}),r.addEventListener("submit",async i=>{i.preventDefault();const a=new FormData(r);Y(10);try{const s=await fetch("/sitemap/upload",{method:"POST",body:a});if(!s.ok){const l=await s.json();throw new Error(l.error||"Failed to upload sitemap")}const{id:o,message:u}=await s.json();Y(50),window.history.pushState({},"",`/results?id=${o}`),_e("/results"),de(u)}catch(s){pt(s.message)}finally{nt()}}),{}}function qs(t,e){let n;return(...r)=>{clearTimeout(n),n=setTimeout(()=>t(...r),e)}}class Ys{constructor(e){zt(this,"render",qs((e,n={},r="url",i="")=>{Y(50),this.urls=e;const a=this.filterUrls(e,n,i);this.treeData=this.buildTree(a,r),this.container.querySelector(".tree-list")||(this.container.innerHTML="",this.renderControls());const s=this.container.querySelector(".tree-list")||document.createElement("ul");s.className="tree-list",s.innerHTML="",this.renderTreeNode(this.treeData,s,"",r),this.container.contains(s)||this.container.appendChild(s),this.setupNodeListeners(),this.updateTreeVisibility(),nt()},200));this.container=e,this.isExpanded=!0,this.expandedNodes=new Set,this.treeData=null,this.urls=[],this.listeners=new Map,this.renderControls()}renderControls(){const e=document.createElement("div");e.className="tree-controls",e.innerHTML=`
    ...
    `,this.container.appendChild(e),this.setupControlListeners()}setupControlListeners(){this.cleanupListeners("controls");const e=this.container.querySelector("#toggleExpansionBtn");if(e){const n=()=>this.toggleExpansion();e.addEventListener("click",n),this.listeners.set(e,[["click",n]])}}toggleExpansion(){this.isExpanded=!this.isExpanded;const e=this.container.querySelector("#toggleExpansionBtn");e.innerHTML=`<i class="fas fa-${this.isExpanded?"compress-alt":"expand-alt"}"></i> ${this.isExpanded?"Collapse All":"Expand All"}`,e.setAttribute("aria-expanded",this.isExpanded),e.setAttribute("aria-label",this.isExpanded?"Collapse all nodes":"Expand all nodes"),this.expandedNodes.clear(),this.isExpanded&&this.buildExpandedNodes(this.treeData,""),this.updateTreeVisibility()}buildExpandedNodes(e,n){Object.entries(e.children).forEach(([r,i])=>{const a=n?`${n}/${i.name}`:i.name;this.expandedNodes.add(a),this.buildExpandedNodes(i,a)})}updateTreeVisibility(){this.container.querySelectorAll(".toggle").forEach(e=>{const n=e.dataset.path,r=this.isExpanded||this.expandedNodes.has(n);e.classList.toggle("open",r),e.setAttribute("aria-expanded",r);const i=e.nextElementSibling;i?(i.classList.toggle("hidden",!r),i.style.display=r?"block":"none"):console.warn(`No child <ul> found for toggle at path: ${n}`)})}setupNodeListeners(){this.cleanupListeners("nodes"),this.container.querySelectorAll(".toggle").forEach(e=>{const n=()=>this.toggleNode(e);e.addEventListener("click",n),e.addEventListener("keydown",r=>r.key==="Enter"&&this.toggleNode(e)),this.listeners.set(e,[["click",n],["keydown",r=>r.key==="Enter"&&this.toggleNode(e)]])})}cleanupListeners(e="all"){this.listeners.forEach((n,r)=>{(e==="all"||e==="controls"&&r.id==="toggleExpansionBtn"||e==="nodes"&&r.classList.contains("toggle"))&&(n.forEach(([i,a])=>r.removeEventListener(i,a)),this.listeners.delete(r))})}filterUrls(e,n,r){return e.filter(i=>{const a=parseFloat(i.priority)||0,s=i.lastmod?new Date(i.lastmod):null,o=parseFloat(n.priority)||0,u=n.lastmod?new Date(n.lastmod):null;return(!r||i.loc.toLowerCase().includes(r.toLowerCase()))&&a>=o&&(!u||s&&s>=u)})}sortUrls(e,n){return e.sort((r,i)=>n==="lastmod"?new Date(i.lastmod||0)-new Date(r.lastmod||0)||r.loc.localeCompare(i.loc):n==="priority"&&parseFloat(i.priority||0)-parseFloat(r.priority||0)||r.loc.localeCompare(i.loc))}buildTree(e,n){const r={children:{},urls:[]};return e.forEach(i=>{try{const a=new URL(i.loc||""),s=a.pathname==="/"?[""]:a.pathname.split("/").filter(l=>l.length);let o=r;const u=a.hostname;o.children[u]||(o.children[u]={name:u,children:{},urls:[]}),o=o.children[u],s.forEach(l=>{const c=l||"/";o.children[c]||(o.children[c]={name:c,children:{},urls:[]}),o=o.children[c]}),o.urls.push(i)}catch(a){console.warn(`Skipping invalid URL: ${i.loc}`,a)}}),this.sortTree(r,n),r}sortTree(e,n){e.urls.length&&(e.urls=this.sortUrls(e.urls,n)),Object.values(e.children).forEach(r=>this.sortTree(r,n))}renderTreeNode(e,n,r,i){Object.entries(e.children).sort(([s,o],[u,l])=>i==="url"?o.name.localeCompare(l.name):0).forEach(([s,o])=>{const u=r?`${r}/${o.name}`:o.name,l=document.createElement("li"),c=document.createElement("span"),f=this.isExpanded||this.expandedNodes.has(u);if(c.className=`toggle ${f?"open":""}`,c.setAttribute("aria-expanded",f),c.setAttribute("aria-label",`Toggle ${o.name} children`),c.setAttribute("role","button"),c.dataset.path=u,c.tabIndex=0,c.textContent=o.name==="/"?u:o.name,l.appendChild(c),o.urls.length||Object.keys(o.children).length){const h=document.createElement("ul");h.className=f?"":"hidden",h.style.display=f?"block":"none",o.urls.forEach(m=>{const v=document.createElement("li");v.innerHTML=`<span class="url-leaf" tabindex="0" aria-label="URL: ${m.loc}">${this.formatUrl(m)}</span>`,h.appendChild(v)}),Object.keys(o.children).length&&this.renderTreeNode(o,h,u,i),l.appendChild(h)}n.appendChild(l)})}formatUrl(e){let n=e.loc;return e.lastmod&&(n+=` (Last Modified: ${e.lastmod})`),e.changefreq&&(n+=`, ${e.changefreq}`),e.priority&&(n+=`, Priority: ${e.priority}`),n}toggleNode(e){const r=!e.classList.contains("open");e.classList.toggle("open",r),e.setAttribute("aria-expanded",r);const i=e.nextElementSibling,a=e.dataset.path;if(i?(i.classList.toggle("hidden",!r),i.style.display=r?"block":"none",console.debug(`Toggled ${a}: ${r?"shown":"hidden"}`)):console.warn(`No child <ul> found for toggle at path: ${a}`),r?this.expandedNodes.add(a):this.expandedNodes.delete(a),this.isExpanded&&!r){this.isExpanded=!1;const s=this.container.querySelector("#toggleExpansionBtn");s.innerHTML='<i class="fas fa-expand-alt"></i> Expand All',s.setAttribute("aria-expanded",!1),s.setAttribute("aria-label","Expand all nodes")}}destroy(){this.cleanupListeners(),this.container.innerHTML="",this.treeData=null,this.urls=[],this.expandedNodes.clear()}}var Xs={value:()=>{}};function Ln(){for(var t=0,e=arguments.length,n={},r;t<e;++t){if(!(r=arguments[t]+"")||r in n||/[\s.]/.test(r))throw new Error("illegal type: "+r);n[r]=[]}return new le(n)}function le(t){this._=t}function Ws(t,e){return t.trim().split(/^|\s+/).map(function(n){var r="",i=n.indexOf(".");if(i>=0&&(r=n.slice(i+1),n=n.slice(0,i)),n&&!e.hasOwnProperty(n))throw new Error("unknown type: "+n);return{type:n,name:r}})}le.prototype=Ln.prototype={constructor:le,on:function(t,e){var n=this._,r=Ws(t+"",n),i,a=-1,s=r.length;if(arguments.length<2){for(;++a<s;)if((i=(t=r[a]).type)&&(i=js(n[i],t.name)))return i;return}if(e!=null&&typeof e!="function")throw new Error("invalid callback: "+e);for(;++a<s;)if(i=(t=r[a]).type)n[i]=or(n[i],t.name,e);else if(e==null)for(i in n)n[i]=or(n[i],t.name,null);return this},copy:function(){var t={},e=this._;for(var n in e)t[n]=e[n].slice();return new le(t)},call:function(t,e){if((i=arguments.length-2)>0)for(var n=new Array(i),r=0,i,a;r<i;++r)n[r]=arguments[r+2];if(!this._.hasOwnProperty(t))throw new Error("unknown type: "+t);for(a=this._[t],r=0,i=a.length;r<i;++r)a[r].value.apply(e,n)},apply:function(t,e,n){if(!this._.hasOwnProperty(t))throw new Error("unknown type: "+t);for(var r=this._[t],i=0,a=r.length;i<a;++i)r[i].value.apply(e,n)}};function js(t,e){for(var n=0,r=t.length,i;n<r;++n)if((i=t[n]).name===e)return i.value}function or(t,e,n){for(var r=0,i=t.length;r<i;++r)if(t[r].name===e){t[r]=Xs,t=t.slice(0,r).concat(t.slice(r+1));break}return n!=null&&t.push({name:e,value:n}),t}var sn="http://www.w3.org/1999/xhtml";const lr={svg:"http://www.w3.org/2000/svg",xhtml:sn,xlink:"http://www.w3.org/1999/xlink",xml:"http://www.w3.org/XML/1998/namespace",xmlns:"http://www.w3.org/2000/xmlns/"};function Ce(t){var e=t+="",n=e.indexOf(":");return n>=0&&(e=t.slice(0,n))!=="xmlns"&&(t=t.slice(n+1)),lr.hasOwnProperty(e)?{space:lr[e],local:t}:t}function Gs(t){return function(){var e=this.ownerDocument,n=this.namespaceURI;return n===sn&&e.documentElement.namespaceURI===sn?e.createElement(t):e.createElementNS(n,t)}}function Ks(t){return function(){return this.ownerDocument.createElementNS(t.space,t.local)}}function ai(t){var e=Ce(t);return(e.local?Ks:Gs)(e)}function Js(){}function Cn(t){return t==null?Js:function(){return this.querySelector(t)}}function Zs(t){typeof t!="function"&&(t=Cn(t));for(var e=this._groups,n=e.length,r=new Array(n),i=0;i<n;++i)for(var a=e[i],s=a.length,o=r[i]=new Array(s),u,l,c=0;c<s;++c)(u=a[c])&&(l=t.call(u,u.__data__,c,a))&&("__data__"in u&&(l.__data__=u.__data__),o[c]=l);return new D(r,this._parents)}function Qs(t){return t==null?[]:Array.isArray(t)?t:Array.from(t)}function to(){return[]}function si(t){return t==null?to:function(){return this.querySelectorAll(t)}}function eo(t){return function(){return Qs(t.apply(this,arguments))}}function no(t){typeof t=="function"?t=eo(t):t=si(t);for(var e=this._groups,n=e.length,r=[],i=[],a=0;a<n;++a)for(var s=e[a],o=s.length,u,l=0;l<o;++l)(u=s[l])&&(r.push(t.call(u,u.__data__,l,s)),i.push(u));return new D(r,i)}function oi(t){return function(){return this.matches(t)}}function li(t){return function(e){return e.matches(t)}}var ro=Array.prototype.find;function io(t){return function(){return ro.call(this.children,t)}}function ao(){return this.firstElementChild}function so(t){return this.select(t==null?ao:io(typeof t=="function"?t:li(t)))}var oo=Array.prototype.filter;function lo(){return Array.from(this.children)}function co(t){return function(){return oo.call(this.children,t)}}function uo(t){return this.selectAll(t==null?lo:co(typeof t=="function"?t:li(t)))}function fo(t){typeof t!="function"&&(t=oi(t));for(var e=this._groups,n=e.length,r=new Array(n),i=0;i<n;++i)for(var a=e[i],s=a.length,o=r[i]=[],u,l=0;l<s;++l)(u=a[l])&&t.call(u,u.__data__,l,a)&&o.push(u);return new D(r,this._parents)}function ci(t){return new Array(t.length)}function ho(){return new D(this._enter||this._groups.map(ci),this._parents)}function ge(t,e){this.ownerDocument=t.ownerDocument,this.namespaceURI=t.namespaceURI,this._next=null,this._parent=t,this.__data__=e}ge.prototype={constructor:ge,appendChild:function(t){return this._parent.insertBefore(t,this._next)},insertBefore:function(t,e){return this._parent.insertBefore(t,e)},querySelector:function(t){return this._parent.querySelector(t)},querySelectorAll:function(t){return this._parent.querySelectorAll(t)}};function mo(t){return function(){return t}}function po(t,e,n,r,i,a){for(var s=0,o,u=e.length,l=a.length;s<l;++s)(o=e[s])?(o.__data__=a[s],r[s]=o):n[s]=new ge(t,a[s]);for(;s<u;++s)(o=e[s])&&(i[s]=o)}function go(t,e,n,r,i,a,s){var o,u,l=new Map,c=e.length,f=a.length,h=new Array(c),m;for(o=0;o<c;++o)(u=e[o])&&(h[o]=m=s.call(u,u.__data__,o,e)+"",l.has(m)?i[o]=u:l.set(m,u));for(o=0;o<f;++o)m=s.call(t,a[o],o,a)+"",(u=l.get(m))?(r[o]=u,u.__data__=a[o],l.delete(m)):n[o]=new ge(t,a[o]);for(o=0;o<c;++o)(u=e[o])&&l.get(h[o])===u&&(i[o]=u)}function yo(t){return t.__data__}function vo(t,e){if(!arguments.length)return Array.from(this,yo);var n=e?go:po,r=this._parents,i=this._groups;typeof t!="function"&&(t=mo(t));for(var a=i.length,s=new Array(a),o=new Array(a),u=new Array(a),l=0;l<a;++l){var c=r[l],f=i[l],h=f.length,m=xo(t.call(c,c&&c.__data__,l,r)),v=m.length,E=o[l]=new Array(v),S=s[l]=new Array(v),y=u[l]=new Array(h);n(c,f,E,S,y,m,e);for(var _=0,A=0,T,P;_<v;++_)if(T=E[_]){for(_>=A&&(A=_+1);!(P=S[A])&&++A<v;);T._next=P||null}}return s=new D(s,r),s._enter=o,s._exit=u,s}function xo(t){return typeof t=="object"&&"length"in t?t:Array.from(t)}function bo(){return new D(this._exit||this._groups.map(ci),this._parents)}function wo(t,e,n){var r=this.enter(),i=this,a=this.exit();return typeof t=="function"?(r=t(r),r&&(r=r.selection())):r=r.append(t+""),e!=null&&(i=e(i),i&&(i=i.selection())),n==null?a.remove():n(a),r&&i?r.merge(i).order():i}function _o(t){for(var e=t.selection?t.selection():t,n=this._groups,r=e._groups,i=n.length,a=r.length,s=Math.min(i,a),o=new Array(i),u=0;u<s;++u)for(var l=n[u],c=r[u],f=l.length,h=o[u]=new Array(f),m,v=0;v<f;++v)(m=l[v]||c[v])&&(h[v]=m);for(;u<i;++u)o[u]=n[u];return new D(o,this._parents)}function Eo(){for(var t=this._groups,e=-1,n=t.length;++e<n;)for(var r=t[e],i=r.length-1,a=r[i],s;--i>=0;)(s=r[i])&&(a&&s.compareDocumentPosition(a)^4&&a.parentNode.insertBefore(s,a),a=s);return this}function ko(t){t||(t=So);function e(f,h){return f&&h?t(f.__data__,h.__data__):!f-!h}for(var n=this._groups,r=n.length,i=new Array(r),a=0;a<r;++a){for(var s=n[a],o=s.length,u=i[a]=new Array(o),l,c=0;c<o;++c)(l=s[c])&&(u[c]=l);u.sort(e)}return new D(i,this._parents).order()}function So(t,e){return t<e?-1:t>e?1:t>=e?0:NaN}function Ao(){var t=arguments[0];return arguments[0]=this,t.apply(null,arguments),this}function Lo(){return Array.from(this)}function Co(){for(var t=this._groups,e=0,n=t.length;e<n;++e)for(var r=t[e],i=0,a=r.length;i<a;++i){var s=r[i];if(s)return s}return null}function To(){let t=0;for(const e of this)++t;return t}function No(){return!this.node()}function Mo(t){for(var e=this._groups,n=0,r=e.length;n<r;++n)for(var i=e[n],a=0,s=i.length,o;a<s;++a)(o=i[a])&&t.call(o,o.__data__,a,i);return this}function Po(t){return function(){this.removeAttribute(t)}}function zo(t){return function(){this.removeAttributeNS(t.space,t.local)}}function Oo(t,e){return function(){this.setAttribute(t,e)}}function Io(t,e){return function(){this.setAttributeNS(t.space,t.local,e)}}function Fo(t,e){return function(){var n=e.apply(this,arguments);n==null?this.removeAttribute(t):this.setAttribute(t,n)}}function $o(t,e){return function(){var n=e.apply(this,arguments);n==null?this.removeAttributeNS(t.space,t.local):this.setAttributeNS(t.space,t.local,n)}}function Ro(t,e){var n=Ce(t);if(arguments.length<2){var r=this.node();return n.local?r.getAttributeNS(n.space,n.local):r.getAttribute(n)}return this.each((e==null?n.local?zo:Po:typeof e=="function"?n.local?$o:Fo:n.local?Io:Oo)(n,e))}function ui(t){return t.ownerDocument&&t.ownerDocument.defaultView||t.document&&t||t.defaultView}function Do(t){return function(){this.style.removeProperty(t)}}function Bo(t,e,n){return function(){this.style.setProperty(t,e,n)}}function Uo(t,e,n){return function(){var r=e.apply(this,arguments);r==null?this.style.removeProperty(t):this.style.setProperty(t,r,n)}}function Ho(t,e,n){return arguments.length>1?this.each((e==null?Do:typeof e=="function"?Uo:Bo)(t,e,n??"")):Nt(this.node(),t)}function Nt(t,e){return t.style.getPropertyValue(e)||ui(t).getComputedStyle(t,null).getPropertyValue(e)}function Vo(t){return function(){delete this[t]}}function qo(t,e){return function(){this[t]=e}}function Yo(t,e){return function(){var n=e.apply(this,arguments);n==null?delete this[t]:this[t]=n}}function Xo(t,e){return arguments.length>1?this.each((e==null?Vo:typeof e=="function"?Yo:qo)(t,e)):this.node()[t]}function fi(t){return t.trim().split(/^|\s+/)}function Tn(t){return t.classList||new hi(t)}function hi(t){this._node=t,this._names=fi(t.getAttribute("class")||"")}hi.prototype={add:function(t){var e=this._names.indexOf(t);e<0&&(this._names.push(t),this._node.setAttribute("class",this._names.join(" ")))},remove:function(t){var e=this._names.indexOf(t);e>=0&&(this._names.splice(e,1),this._node.setAttribute("class",this._names.join(" ")))},contains:function(t){return this._names.indexOf(t)>=0}};function di(t,e){for(var n=Tn(t),r=-1,i=e.length;++r<i;)n.add(e[r])}function mi(t,e){for(var n=Tn(t),r=-1,i=e.length;++r<i;)n.remove(e[r])}function Wo(t){return function(){di(this,t)}}function jo(t){return function(){mi(this,t)}}function Go(t,e){return function(){(e.apply(this,arguments)?di:mi)(this,t)}}function Ko(t,e){var n=fi(t+"");if(arguments.length<2){for(var r=Tn(this.node()),i=-1,a=n.length;++i<a;)if(!r.contains(n[i]))return!1;return!0}return this.each((typeof e=="function"?Go:e?Wo:jo)(n,e))}function Jo(){this.textContent=""}function Zo(t){return function(){this.textContent=t}}function Qo(t){return function(){var e=t.apply(this,arguments);this.textContent=e??""}}function tl(t){return arguments.length?this.each(t==null?Jo:(typeof t=="function"?Qo:Zo)(t)):this.node().textContent}function el(){this.innerHTML=""}function nl(t){return function(){this.innerHTML=t}}function rl(t){return function(){var e=t.apply(this,arguments);this.innerHTML=e??""}}function il(t){return arguments.length?this.each(t==null?el:(typeof t=="function"?rl:nl)(t)):this.node().innerHTML}function al(){this.nextSibling&&this.parentNode.appendChild(this)}function sl(){return this.each(al)}function ol(){this.previousSibling&&this.parentNode.insertBefore(this,this.parentNode.firstChild)}function ll(){return this.each(ol)}function cl(t){var e=typeof t=="function"?t:ai(t);return this.select(function(){return this.appendChild(e.apply(this,arguments))})}function ul(){return null}function fl(t,e){var n=typeof t=="function"?t:ai(t),r=e==null?ul:typeof e=="function"?e:Cn(e);return this.select(function(){return this.insertBefore(n.apply(this,arguments),r.apply(this,arguments)||null)})}function hl(){var t=this.parentNode;t&&t.removeChild(this)}function dl(){return this.each(hl)}function ml(){var t=this.cloneNode(!1),e=this.parentNode;return e?e.insertBefore(t,this.nextSibling):t}function pl(){var t=this.cloneNode(!0),e=this.parentNode;return e?e.insertBefore(t,this.nextSibling):t}function gl(t){return this.select(t?pl:ml)}function yl(t){return arguments.length?this.property("__data__",t):this.node().__data__}function vl(t){return function(e){t.call(this,e,this.__data__)}}function xl(t){return t.trim().split(/^|\s+/).map(function(e){var n="",r=e.indexOf(".");return r>=0&&(n=e.slice(r+1),e=e.slice(0,r)),{type:e,name:n}})}function bl(t){return function(){var e=this.__on;if(e){for(var n=0,r=-1,i=e.length,a;n<i;++n)a=e[n],(!t.type||a.type===t.type)&&a.name===t.name?this.removeEventListener(a.type,a.listener,a.options):e[++r]=a;++r?e.length=r:delete this.__on}}}function wl(t,e,n){return function(){var r=this.__on,i,a=vl(e);if(r){for(var s=0,o=r.length;s<o;++s)if((i=r[s]).type===t.type&&i.name===t.name){this.removeEventListener(i.type,i.listener,i.options),this.addEventListener(i.type,i.listener=a,i.options=n),i.value=e;return}}this.addEventListener(t.type,a,n),i={type:t.type,name:t.name,value:e,listener:a,options:n},r?r.push(i):this.__on=[i]}}function _l(t,e,n){var r=xl(t+""),i,a=r.length,s;if(arguments.length<2){var o=this.node().__on;if(o){for(var u=0,l=o.length,c;u<l;++u)for(i=0,c=o[u];i<a;++i)if((s=r[i]).type===c.type&&s.name===c.name)return c.value}return}for(o=e?wl:bl,i=0;i<a;++i)this.each(o(r[i],e,n));return this}function pi(t,e,n){var r=ui(t),i=r.CustomEvent;typeof i=="function"?i=new i(e,n):(i=r.document.createEvent("Event"),n?(i.initEvent(e,n.bubbles,n.cancelable),i.detail=n.detail):i.initEvent(e,!1,!1)),t.dispatchEvent(i)}function El(t,e){return function(){return pi(this,t,e)}}function kl(t,e){return function(){return pi(this,t,e.apply(this,arguments))}}function Sl(t,e){return this.each((typeof e=="function"?kl:El)(t,e))}function*Al(){for(var t=this._groups,e=0,n=t.length;e<n;++e)for(var r=t[e],i=0,a=r.length,s;i<a;++i)(s=r[i])&&(yield s)}var gi=[null];function D(t,e){this._groups=t,this._parents=e}function Kt(){return new D([[document.documentElement]],gi)}function Ll(){return this}D.prototype=Kt.prototype={constructor:D,select:Zs,selectAll:no,selectChild:so,selectChildren:uo,filter:fo,data:vo,enter:ho,exit:bo,join:wo,merge:_o,selection:Ll,order:Eo,sort:ko,call:Ao,nodes:Lo,node:Co,size:To,empty:No,each:Mo,attr:Ro,style:Ho,property:Xo,classed:Ko,text:tl,html:il,raise:sl,lower:ll,append:cl,insert:fl,remove:dl,clone:gl,datum:yl,on:_l,dispatch:Sl,[Symbol.iterator]:Al};function ct(t){return typeof t=="string"?new D([[document.querySelector(t)]],[document.documentElement]):new D([[t]],gi)}function Cl(t){let e;for(;e=t.sourceEvent;)t=e;return t}function dt(t,e){if(t=Cl(t),e===void 0&&(e=t.currentTarget),e){var n=e.ownerSVGElement||e;if(n.createSVGPoint){var r=n.createSVGPoint();return r.x=t.clientX,r.y=t.clientY,r=r.matrixTransform(e.getScreenCTM().inverse()),[r.x,r.y]}if(e.getBoundingClientRect){var i=e.getBoundingClientRect();return[t.clientX-i.left-e.clientLeft,t.clientY-i.top-e.clientTop]}}return[t.pageX,t.pageY]}const on={capture:!0,passive:!1};function ln(t){t.preventDefault(),t.stopImmediatePropagation()}function Tl(t){var e=t.document.documentElement,n=ct(t).on("dragstart.drag",ln,on);"onselectstart"in e?n.on("selectstart.drag",ln,on):(e.__noselect=e.style.MozUserSelect,e.style.MozUserSelect="none")}function Nl(t,e){var n=t.document.documentElement,r=ct(t).on("dragstart.drag",null);e&&(r.on("click.drag",ln,on),setTimeout(function(){r.on("click.drag",null)},0)),"onselectstart"in n?r.on("selectstart.drag",null):(n.style.MozUserSelect=n.__noselect,delete n.__noselect)}function Nn(t,e,n){t.prototype=e.prototype=n,n.constructor=t}function yi(t,e){var n=Object.create(t.prototype);for(var r in e)n[r]=e[r];return n}function Jt(){}var Vt=.7,ye=1/Vt,Lt="\\s*([+-]?\\d+)\\s*",qt="\\s*([+-]?(?:\\d*\\.)?\\d+(?:[eE][+-]?\\d+)?)\\s*",K="\\s*([+-]?(?:\\d*\\.)?\\d+(?:[eE][+-]?\\d+)?)%\\s*",Ml=/^#([0-9a-f]{3,8})$/,Pl=new RegExp(`^rgb\\(${Lt},${Lt},${Lt}\\)$`),zl=new RegExp(`^rgb\\(${K},${K},${K}\\)$`),Ol=new RegExp(`^rgba\\(${Lt},${Lt},${Lt},${qt}\\)$`),Il=new RegExp(`^rgba\\(${K},${K},${K},${qt}\\)$`),Fl=new RegExp(`^hsl\\(${qt},${K},${K}\\)$`),$l=new RegExp(`^hsla\\(${qt},${K},${K},${qt}\\)$`),cr={aliceblue:15792383,antiquewhite:16444375,aqua:65535,aquamarine:8388564,azure:15794175,beige:16119260,bisque:16770244,black:0,blanchedalmond:16772045,blue:255,blueviolet:9055202,brown:10824234,burlywood:14596231,cadetblue:6266528,chartreuse:8388352,chocolate:13789470,coral:16744272,cornflowerblue:6591981,cornsilk:16775388,crimson:14423100,cyan:65535,darkblue:139,darkcyan:35723,darkgoldenrod:12092939,darkgray:11119017,darkgreen:25600,darkgrey:11119017,darkkhaki:12433259,darkmagenta:9109643,darkolivegreen:5597999,darkorange:16747520,darkorchid:10040012,darkred:9109504,darksalmon:15308410,darkseagreen:9419919,darkslateblue:4734347,darkslategray:3100495,darkslategrey:3100495,darkturquoise:52945,darkviolet:9699539,deeppink:16716947,deepskyblue:49151,dimgray:6908265,dimgrey:6908265,dodgerblue:2003199,firebrick:11674146,floralwhite:16775920,forestgreen:2263842,fuchsia:16711935,gainsboro:14474460,ghostwhite:16316671,gold:16766720,goldenrod:14329120,gray:8421504,green:32768,greenyellow:11403055,grey:8421504,honeydew:15794160,hotpink:16738740,indianred:13458524,indigo:4915330,ivory:16777200,khaki:15787660,lavender:15132410,lavenderblush:16773365,lawngreen:8190976,lemonchiffon:16775885,lightblue:11393254,lightcoral:15761536,lightcyan:14745599,lightgoldenrodyellow:16448210,lightgray:13882323,lightgreen:9498256,lightgrey:13882323,lightpink:16758465,lightsalmon:16752762,lightseagreen:2142890,lightskyblue:8900346,lightslategray:7833753,lightslategrey:7833753,lightsteelblue:11584734,lightyellow:16777184,lime:65280,limegreen:3329330,linen:16445670,magenta:16711935,maroon:8388608,mediumaquamarine:6737322,mediumblue:205,mediumorchid:12211667,mediumpurple:9662683,mediumseagreen:3978097,mediumslateblue:8087790,mediumspringgreen:64154,mediumturquoise:4772300,mediumvioletred:13047173,midnightblue:1644912,mintcream:16121850,mistyrose:16770273,moccasin:16770229,navajowhite:16768685,navy:128,oldlace:16643558,olive:8421376,olivedrab:7048739,orange:16753920,orangered:16729344,orchid:14315734,palegoldenrod:15657130,palegreen:10025880,paleturquoise:11529966,palevioletred:14381203,papayawhip:16773077,peachpuff:16767673,peru:13468991,pink:16761035,plum:14524637,powderblue:11591910,purple:8388736,rebeccapurple:6697881,red:16711680,rosybrown:12357519,royalblue:4286945,saddlebrown:9127187,salmon:16416882,sandybrown:16032864,seagreen:3050327,seashell:16774638,sienna:10506797,silver:12632256,skyblue:8900331,slateblue:6970061,slategray:7372944,slategrey:7372944,snow:16775930,springgreen:65407,steelblue:4620980,tan:13808780,teal:32896,thistle:14204888,tomato:16737095,turquoise:4251856,violet:15631086,wheat:16113331,white:16777215,whitesmoke:16119285,yellow:16776960,yellowgreen:10145074};Nn(Jt,Yt,{copy(t){return Object.assign(new this.constructor,this,t)},displayable(){return this.rgb().displayable()},hex:ur,formatHex:ur,formatHex8:Rl,formatHsl:Dl,formatRgb:fr,toString:fr});function ur(){return this.rgb().formatHex()}function Rl(){return this.rgb().formatHex8()}function Dl(){return vi(this).formatHsl()}function fr(){return this.rgb().formatRgb()}function Yt(t){var e,n;return t=(t+"").trim().toLowerCase(),(e=Ml.exec(t))?(n=e[1].length,e=parseInt(e[1],16),n===6?hr(e):n===3?new $(e>>8&15|e>>4&240,e>>4&15|e&240,(e&15)<<4|e&15,1):n===8?ne(e>>24&255,e>>16&255,e>>8&255,(e&255)/255):n===4?ne(e>>12&15|e>>8&240,e>>8&15|e>>4&240,e>>4&15|e&240,((e&15)<<4|e&15)/255):null):(e=Pl.exec(t))?new $(e[1],e[2],e[3],1):(e=zl.exec(t))?new $(e[1]*255/100,e[2]*255/100,e[3]*255/100,1):(e=Ol.exec(t))?ne(e[1],e[2],e[3],e[4]):(e=Il.exec(t))?ne(e[1]*255/100,e[2]*255/100,e[3]*255/100,e[4]):(e=Fl.exec(t))?pr(e[1],e[2]/100,e[3]/100,1):(e=$l.exec(t))?pr(e[1],e[2]/100,e[3]/100,e[4]):cr.hasOwnProperty(t)?hr(cr[t]):t==="transparent"?new $(NaN,NaN,NaN,0):null}function hr(t){return new $(t>>16&255,t>>8&255,t&255,1)}function ne(t,e,n,r){return r<=0&&(t=e=n=NaN),new $(t,e,n,r)}function Bl(t){return t instanceof Jt||(t=Yt(t)),t?(t=t.rgb(),new $(t.r,t.g,t.b,t.opacity)):new $}function cn(t,e,n,r){return arguments.length===1?Bl(t):new $(t,e,n,r??1)}function $(t,e,n,r){this.r=+t,this.g=+e,this.b=+n,this.opacity=+r}Nn($,cn,yi(Jt,{brighter(t){return t=t==null?ye:Math.pow(ye,t),new $(this.r*t,this.g*t,this.b*t,this.opacity)},darker(t){return t=t==null?Vt:Math.pow(Vt,t),new $(this.r*t,this.g*t,this.b*t,this.opacity)},rgb(){return this},clamp(){return new $(vt(this.r),vt(this.g),vt(this.b),ve(this.opacity))},displayable(){return-.5<=this.r&&this.r<255.5&&-.5<=this.g&&this.g<255.5&&-.5<=this.b&&this.b<255.5&&0<=this.opacity&&this.opacity<=1},hex:dr,formatHex:dr,formatHex8:Ul,formatRgb:mr,toString:mr}));function dr(){return`#${yt(this.r)}${yt(this.g)}${yt(this.b)}`}function Ul(){return`#${yt(this.r)}${yt(this.g)}${yt(this.b)}${yt((isNaN(this.opacity)?1:this.opacity)*255)}`}function mr(){const t=ve(this.opacity);return`${t===1?"rgb(":"rgba("}${vt(this.r)}, ${vt(this.g)}, ${vt(this.b)}${t===1?")":`, ${t})`}`}function ve(t){return isNaN(t)?1:Math.max(0,Math.min(1,t))}function vt(t){return Math.max(0,Math.min(255,Math.round(t)||0))}function yt(t){return t=vt(t),(t<16?"0":"")+t.toString(16)}function pr(t,e,n,r){return r<=0?t=e=n=NaN:n<=0||n>=1?t=e=NaN:e<=0&&(t=NaN),new q(t,e,n,r)}function vi(t){if(t instanceof q)return new q(t.h,t.s,t.l,t.opacity);if(t instanceof Jt||(t=Yt(t)),!t)return new q;if(t instanceof q)return t;t=t.rgb();var e=t.r/255,n=t.g/255,r=t.b/255,i=Math.min(e,n,r),a=Math.max(e,n,r),s=NaN,o=a-i,u=(a+i)/2;return o?(e===a?s=(n-r)/o+(n<r)*6:n===a?s=(r-e)/o+2:s=(e-n)/o+4,o/=u<.5?a+i:2-a-i,s*=60):o=u>0&&u<1?0:s,new q(s,o,u,t.opacity)}function Hl(t,e,n,r){return arguments.length===1?vi(t):new q(t,e,n,r??1)}function q(t,e,n,r){this.h=+t,this.s=+e,this.l=+n,this.opacity=+r}Nn(q,Hl,yi(Jt,{brighter(t){return t=t==null?ye:Math.pow(ye,t),new q(this.h,this.s,this.l*t,this.opacity)},darker(t){return t=t==null?Vt:Math.pow(Vt,t),new q(this.h,this.s,this.l*t,this.opacity)},rgb(){var t=this.h%360+(this.h<0)*360,e=isNaN(t)||isNaN(this.s)?0:this.s,n=this.l,r=n+(n<.5?n:1-n)*e,i=2*n-r;return new $(Re(t>=240?t-240:t+120,i,r),Re(t,i,r),Re(t<120?t+240:t-120,i,r),this.opacity)},clamp(){return new q(gr(this.h),re(this.s),re(this.l),ve(this.opacity))},displayable(){return(0<=this.s&&this.s<=1||isNaN(this.s))&&0<=this.l&&this.l<=1&&0<=this.opacity&&this.opacity<=1},formatHsl(){const t=ve(this.opacity);return`${t===1?"hsl(":"hsla("}${gr(this.h)}, ${re(this.s)*100}%, ${re(this.l)*100}%${t===1?")":`, ${t})`}`}}));function gr(t){return t=(t||0)%360,t<0?t+360:t}function re(t){return Math.max(0,Math.min(1,t||0))}function Re(t,e,n){return(t<60?e+(n-e)*t/60:t<180?n:t<240?e+(n-e)*(240-t)/60:e)*255}const xi=t=>()=>t;function Vl(t,e){return function(n){return t+n*e}}function ql(t,e,n){return t=Math.pow(t,n),e=Math.pow(e,n)-t,n=1/n,function(r){return Math.pow(t+r*e,n)}}function Yl(t){return(t=+t)==1?bi:function(e,n){return n-e?ql(e,n,t):xi(isNaN(e)?n:e)}}function bi(t,e){var n=e-t;return n?Vl(t,n):xi(isNaN(t)?e:t)}const yr=function t(e){var n=Yl(e);function r(i,a){var s=n((i=cn(i)).r,(a=cn(a)).r),o=n(i.g,a.g),u=n(i.b,a.b),l=bi(i.opacity,a.opacity);return function(c){return i.r=s(c),i.g=o(c),i.b=u(c),i.opacity=l(c),i+""}}return r.gamma=t,r}(1);function lt(t,e){return t=+t,e=+e,function(n){return t*(1-n)+e*n}}var un=/[-+]?(?:\d+\.?\d*|\.?\d+)(?:[eE][-+]?\d+)?/g,De=new RegExp(un.source,"g");function Xl(t){return function(){return t}}function Wl(t){return function(e){return t(e)+""}}function jl(t,e){var n=un.lastIndex=De.lastIndex=0,r,i,a,s=-1,o=[],u=[];for(t=t+"",e=e+"";(r=un.exec(t))&&(i=De.exec(e));)(a=i.index)>n&&(a=e.slice(n,a),o[s]?o[s]+=a:o[++s]=a),(r=r[0])===(i=i[0])?o[s]?o[s]+=i:o[++s]=i:(o[++s]=null,u.push({i:s,x:lt(r,i)})),n=De.lastIndex;return n<e.length&&(a=e.slice(n),o[s]?o[s]+=a:o[++s]=a),o.length<2?u[0]?Wl(u[0].x):Xl(e):(e=u.length,function(l){for(var c=0,f;c<e;++c)o[(f=u[c]).i]=f.x(l);return o.join("")})}var vr=180/Math.PI,fn={translateX:0,translateY:0,rotate:0,skewX:0,scaleX:1,scaleY:1};function wi(t,e,n,r,i,a){var s,o,u;return(s=Math.sqrt(t*t+e*e))&&(t/=s,e/=s),(u=t*n+e*r)&&(n-=t*u,r-=e*u),(o=Math.sqrt(n*n+r*r))&&(n/=o,r/=o,u/=o),t*r<e*n&&(t=-t,e=-e,u=-u,s=-s),{translateX:i,translateY:a,rotate:Math.atan2(e,t)*vr,skewX:Math.atan(u)*vr,scaleX:s,scaleY:o}}var ie;function Gl(t){const e=new(typeof DOMMatrix=="function"?DOMMatrix:WebKitCSSMatrix)(t+"");return e.isIdentity?fn:wi(e.a,e.b,e.c,e.d,e.e,e.f)}function Kl(t){return t==null||(ie||(ie=document.createElementNS("http://www.w3.org/2000/svg","g")),ie.setAttribute("transform",t),!(t=ie.transform.baseVal.consolidate()))?fn:(t=t.matrix,wi(t.a,t.b,t.c,t.d,t.e,t.f))}function _i(t,e,n,r){function i(l){return l.length?l.pop()+" ":""}function a(l,c,f,h,m,v){if(l!==f||c!==h){var E=m.push("translate(",null,e,null,n);v.push({i:E-4,x:lt(l,f)},{i:E-2,x:lt(c,h)})}else(f||h)&&m.push("translate("+f+e+h+n)}function s(l,c,f,h){l!==c?(l-c>180?c+=360:c-l>180&&(l+=360),h.push({i:f.push(i(f)+"rotate(",null,r)-2,x:lt(l,c)})):c&&f.push(i(f)+"rotate("+c+r)}function o(l,c,f,h){l!==c?h.push({i:f.push(i(f)+"skewX(",null,r)-2,x:lt(l,c)}):c&&f.push(i(f)+"skewX("+c+r)}function u(l,c,f,h,m,v){if(l!==f||c!==h){var E=m.push(i(m)+"scale(",null,",",null,")");v.push({i:E-4,x:lt(l,f)},{i:E-2,x:lt(c,h)})}else(f!==1||h!==1)&&m.push(i(m)+"scale("+f+","+h+")")}return function(l,c){var f=[],h=[];return l=t(l),c=t(c),a(l.translateX,l.translateY,c.translateX,c.translateY,f,h),s(l.rotate,c.rotate,f,h),o(l.skewX,c.skewX,f,h),u(l.scaleX,l.scaleY,c.scaleX,c.scaleY,f,h),l=c=null,function(m){for(var v=-1,E=h.length,S;++v<E;)f[(S=h[v]).i]=S.x(m);return f.join("")}}}var Jl=_i(Gl,"px, ","px)","deg)"),Zl=_i(Kl,", ",")",")"),Ql=1e-12;function xr(t){return((t=Math.exp(t))+1/t)/2}function tc(t){return((t=Math.exp(t))-1/t)/2}function ec(t){return((t=Math.exp(2*t))-1)/(t+1)}const nc=function t(e,n,r){function i(a,s){var o=a[0],u=a[1],l=a[2],c=s[0],f=s[1],h=s[2],m=c-o,v=f-u,E=m*m+v*v,S,y;if(E<Ql)y=Math.log(h/l)/e,S=function(U){return[o+U*m,u+U*v,l*Math.exp(e*U*y)]};else{var _=Math.sqrt(E),A=(h*h-l*l+r*E)/(2*l*n*_),T=(h*h-l*l-r*E)/(2*h*n*_),P=Math.log(Math.sqrt(A*A+1)-A),O=Math.log(Math.sqrt(T*T+1)-T);y=(O-P)/e,S=function(U){var _t=U*y,Zt=xr(P),Qt=l/(n*_)*(Zt*ec(e*_t+P)-tc(P));return[o+Qt*m,u+Qt*v,l*Zt/xr(e*_t+P)]}}return S.duration=y*1e3*e/Math.SQRT2,S}return i.rho=function(a){var s=Math.max(.001,+a),o=s*s,u=o*o;return t(s,o,u)},i}(Math.SQRT2,2,4);var Mt=0,$t=0,Ot=0,Ei=1e3,xe,Rt,be=0,wt=0,Te=0,Xt=typeof performance=="object"&&performance.now?performance:Date,ki=typeof window=="object"&&window.requestAnimationFrame?window.requestAnimationFrame.bind(window):function(t){setTimeout(t,17)};function Mn(){return wt||(ki(rc),wt=Xt.now()+Te)}function rc(){wt=0}function we(){this._call=this._time=this._next=null}we.prototype=Si.prototype={constructor:we,restart:function(t,e,n){if(typeof t!="function")throw new TypeError("callback is not a function");n=(n==null?Mn():+n)+(e==null?0:+e),!this._next&&Rt!==this&&(Rt?Rt._next=this:xe=this,Rt=this),this._call=t,this._time=n,hn()},stop:function(){this._call&&(this._call=null,this._time=1/0,hn())}};function Si(t,e,n){var r=new we;return r.restart(t,e,n),r}function ic(){Mn(),++Mt;for(var t=xe,e;t;)(e=wt-t._time)>=0&&t._call.call(void 0,e),t=t._next;--Mt}function br(){wt=(be=Xt.now())+Te,Mt=$t=0;try{ic()}finally{Mt=0,sc(),wt=0}}function ac(){var t=Xt.now(),e=t-be;e>Ei&&(Te-=e,be=t)}function sc(){for(var t,e=xe,n,r=1/0;e;)e._call?(r>e._time&&(r=e._time),t=e,e=e._next):(n=e._next,e._next=null,e=t?t._next=n:xe=n);Rt=t,hn(r)}function hn(t){if(!Mt){$t&&($t=clearTimeout($t));var e=t-wt;e>24?(t<1/0&&($t=setTimeout(br,t-Xt.now()-Te)),Ot&&(Ot=clearInterval(Ot))):(Ot||(be=Xt.now(),Ot=setInterval(ac,Ei)),Mt=1,ki(br))}}function wr(t,e,n){var r=new we;return e=e==null?0:+e,r.restart(i=>{r.stop(),t(i+e)},e,n),r}var oc=Ln("start","end","cancel","interrupt"),lc=[],Ai=0,_r=1,dn=2,ce=3,Er=4,mn=5,ue=6;function Ne(t,e,n,r,i,a){var s=t.__transition;if(!s)t.__transition={};else if(n in s)return;cc(t,n,{name:e,index:r,group:i,on:oc,tween:lc,time:a.time,delay:a.delay,duration:a.duration,ease:a.ease,timer:null,state:Ai})}function Pn(t,e){var n=X(t,e);if(n.state>Ai)throw new Error("too late; already scheduled");return n}function J(t,e){var n=X(t,e);if(n.state>ce)throw new Error("too late; already running");return n}function X(t,e){var n=t.__transition;if(!n||!(n=n[e]))throw new Error("transition not found");return n}function cc(t,e,n){var r=t.__transition,i;r[e]=n,n.timer=Si(a,0,n.time);function a(l){n.state=_r,n.timer.restart(s,n.delay,n.time),n.delay<=l&&s(l-n.delay)}function s(l){var c,f,h,m;if(n.state!==_r)return u();for(c in r)if(m=r[c],m.name===n.name){if(m.state===ce)return wr(s);m.state===Er?(m.state=ue,m.timer.stop(),m.on.call("interrupt",t,t.__data__,m.index,m.group),delete r[c]):+c<e&&(m.state=ue,m.timer.stop(),m.on.call("cancel",t,t.__data__,m.index,m.group),delete r[c])}if(wr(function(){n.state===ce&&(n.state=Er,n.timer.restart(o,n.delay,n.time),o(l))}),n.state=dn,n.on.call("start",t,t.__data__,n.index,n.group),n.state===dn){for(n.state=ce,i=new Array(h=n.tween.length),c=0,f=-1;c<h;++c)(m=n.tween[c].value.call(t,t.__data__,n.index,n.group))&&(i[++f]=m);i.length=f+1}}function o(l){for(var c=l<n.duration?n.ease.call(null,l/n.duration):(n.timer.restart(u),n.state=mn,1),f=-1,h=i.length;++f<h;)i[f].call(t,c);n.state===mn&&(n.on.call("end",t,t.__data__,n.index,n.group),u())}function u(){n.state=ue,n.timer.stop(),delete r[e];for(var l in r)return;delete t.__transition}}function fe(t,e){var n=t.__transition,r,i,a=!0,s;if(n){e=e==null?null:e+"";for(s in n){if((r=n[s]).name!==e){a=!1;continue}i=r.state>dn&&r.state<mn,r.state=ue,r.timer.stop(),r.on.call(i?"interrupt":"cancel",t,t.__data__,r.index,r.group),delete n[s]}a&&delete t.__transition}}function uc(t){return this.each(function(){fe(this,t)})}function fc(t,e){var n,r;return function(){var i=J(this,t),a=i.tween;if(a!==n){r=n=a;for(var s=0,o=r.length;s<o;++s)if(r[s].name===e){r=r.slice(),r.splice(s,1);break}}i.tween=r}}function hc(t,e,n){var r,i;if(typeof n!="function")throw new Error;return function(){var a=J(this,t),s=a.tween;if(s!==r){i=(r=s).slice();for(var o={name:e,value:n},u=0,l=i.length;u<l;++u)if(i[u].name===e){i[u]=o;break}u===l&&i.push(o)}a.tween=i}}function dc(t,e){var n=this._id;if(t+="",arguments.length<2){for(var r=X(this.node(),n).tween,i=0,a=r.length,s;i<a;++i)if((s=r[i]).name===t)return s.value;return null}return this.each((e==null?fc:hc)(n,t,e))}function zn(t,e,n){var r=t._id;return t.each(function(){var i=J(this,r);(i.value||(i.value={}))[e]=n.apply(this,arguments)}),function(i){return X(i,r).value[e]}}function Li(t,e){var n;return(typeof e=="number"?lt:e instanceof Yt?yr:(n=Yt(e))?(e=n,yr):jl)(t,e)}function mc(t){return function(){this.removeAttribute(t)}}function pc(t){return function(){this.removeAttributeNS(t.space,t.local)}}function gc(t,e,n){var r,i=n+"",a;return function(){var s=this.getAttribute(t);return s===i?null:s===r?a:a=e(r=s,n)}}function yc(t,e,n){var r,i=n+"",a;return function(){var s=this.getAttributeNS(t.space,t.local);return s===i?null:s===r?a:a=e(r=s,n)}}function vc(t,e,n){var r,i,a;return function(){var s,o=n(this),u;return o==null?void this.removeAttribute(t):(s=this.getAttribute(t),u=o+"",s===u?null:s===r&&u===i?a:(i=u,a=e(r=s,o)))}}function xc(t,e,n){var r,i,a;return function(){var s,o=n(this),u;return o==null?void this.removeAttributeNS(t.space,t.local):(s=this.getAttributeNS(t.space,t.local),u=o+"",s===u?null:s===r&&u===i?a:(i=u,a=e(r=s,o)))}}function bc(t,e){var n=Ce(t),r=n==="transform"?Zl:Li;return this.attrTween(t,typeof e=="function"?(n.local?xc:vc)(n,r,zn(this,"attr."+t,e)):e==null?(n.local?pc:mc)(n):(n.local?yc:gc)(n,r,e))}function wc(t,e){return function(n){this.setAttribute(t,e.call(this,n))}}function _c(t,e){return function(n){this.setAttributeNS(t.space,t.local,e.call(this,n))}}function Ec(t,e){var n,r;function i(){var a=e.apply(this,arguments);return a!==r&&(n=(r=a)&&_c(t,a)),n}return i._value=e,i}function kc(t,e){var n,r;function i(){var a=e.apply(this,arguments);return a!==r&&(n=(r=a)&&wc(t,a)),n}return i._value=e,i}function Sc(t,e){var n="attr."+t;if(arguments.length<2)return(n=this.tween(n))&&n._value;if(e==null)return this.tween(n,null);if(typeof e!="function")throw new Error;var r=Ce(t);return this.tween(n,(r.local?Ec:kc)(r,e))}function Ac(t,e){return function(){Pn(this,t).delay=+e.apply(this,arguments)}}function Lc(t,e){return e=+e,function(){Pn(this,t).delay=e}}function Cc(t){var e=this._id;return arguments.length?this.each((typeof t=="function"?Ac:Lc)(e,t)):X(this.node(),e).delay}function Tc(t,e){return function(){J(this,t).duration=+e.apply(this,arguments)}}function Nc(t,e){return e=+e,function(){J(this,t).duration=e}}function Mc(t){var e=this._id;return arguments.length?this.each((typeof t=="function"?Tc:Nc)(e,t)):X(this.node(),e).duration}function Pc(t,e){if(typeof e!="function")throw new Error;return function(){J(this,t).ease=e}}function zc(t){var e=this._id;return arguments.length?this.each(Pc(e,t)):X(this.node(),e).ease}function Oc(t,e){return function(){var n=e.apply(this,arguments);if(typeof n!="function")throw new Error;J(this,t).ease=n}}function Ic(t){if(typeof t!="function")throw new Error;return this.each(Oc(this._id,t))}function Fc(t){typeof t!="function"&&(t=oi(t));for(var e=this._groups,n=e.length,r=new Array(n),i=0;i<n;++i)for(var a=e[i],s=a.length,o=r[i]=[],u,l=0;l<s;++l)(u=a[l])&&t.call(u,u.__data__,l,a)&&o.push(u);return new at(r,this._parents,this._name,this._id)}function $c(t){if(t._id!==this._id)throw new Error;for(var e=this._groups,n=t._groups,r=e.length,i=n.length,a=Math.min(r,i),s=new Array(r),o=0;o<a;++o)for(var u=e[o],l=n[o],c=u.length,f=s[o]=new Array(c),h,m=0;m<c;++m)(h=u[m]||l[m])&&(f[m]=h);for(;o<r;++o)s[o]=e[o];return new at(s,this._parents,this._name,this._id)}function Rc(t){return(t+"").trim().split(/^|\s+/).every(function(e){var n=e.indexOf(".");return n>=0&&(e=e.slice(0,n)),!e||e==="start"})}function Dc(t,e,n){var r,i,a=Rc(e)?Pn:J;return function(){var s=a(this,t),o=s.on;o!==r&&(i=(r=o).copy()).on(e,n),s.on=i}}function Bc(t,e){var n=this._id;return arguments.length<2?X(this.node(),n).on.on(t):this.each(Dc(n,t,e))}function Uc(t){return function(){var e=this.parentNode;for(var n in this.__transition)if(+n!==t)return;e&&e.removeChild(this)}}function Hc(){return this.on("end.remove",Uc(this._id))}function Vc(t){var e=this._name,n=this._id;typeof t!="function"&&(t=Cn(t));for(var r=this._groups,i=r.length,a=new Array(i),s=0;s<i;++s)for(var o=r[s],u=o.length,l=a[s]=new Array(u),c,f,h=0;h<u;++h)(c=o[h])&&(f=t.call(c,c.__data__,h,o))&&("__data__"in c&&(f.__data__=c.__data__),l[h]=f,Ne(l[h],e,n,h,l,X(c,n)));return new at(a,this._parents,e,n)}function qc(t){var e=this._name,n=this._id;typeof t!="function"&&(t=si(t));for(var r=this._groups,i=r.length,a=[],s=[],o=0;o<i;++o)for(var u=r[o],l=u.length,c,f=0;f<l;++f)if(c=u[f]){for(var h=t.call(c,c.__data__,f,u),m,v=X(c,n),E=0,S=h.length;E<S;++E)(m=h[E])&&Ne(m,e,n,E,h,v);a.push(h),s.push(c)}return new at(a,s,e,n)}var Yc=Kt.prototype.constructor;function Xc(){return new Yc(this._groups,this._parents)}function Wc(t,e){var n,r,i;return function(){var a=Nt(this,t),s=(this.style.removeProperty(t),Nt(this,t));return a===s?null:a===n&&s===r?i:i=e(n=a,r=s)}}function Ci(t){return function(){this.style.removeProperty(t)}}function jc(t,e,n){var r,i=n+"",a;return function(){var s=Nt(this,t);return s===i?null:s===r?a:a=e(r=s,n)}}function Gc(t,e,n){var r,i,a;return function(){var s=Nt(this,t),o=n(this),u=o+"";return o==null&&(u=o=(this.style.removeProperty(t),Nt(this,t))),s===u?null:s===r&&u===i?a:(i=u,a=e(r=s,o))}}function Kc(t,e){var n,r,i,a="style."+e,s="end."+a,o;return function(){var u=J(this,t),l=u.on,c=u.value[a]==null?o||(o=Ci(e)):void 0;(l!==n||i!==c)&&(r=(n=l).copy()).on(s,i=c),u.on=r}}function Jc(t,e,n){var r=(t+="")=="transform"?Jl:Li;return e==null?this.styleTween(t,Wc(t,r)).on("end.style."+t,Ci(t)):typeof e=="function"?this.styleTween(t,Gc(t,r,zn(this,"style."+t,e))).each(Kc(this._id,t)):this.styleTween(t,jc(t,r,e),n).on("end.style."+t,null)}function Zc(t,e,n){return function(r){this.style.setProperty(t,e.call(this,r),n)}}function Qc(t,e,n){var r,i;function a(){var s=e.apply(this,arguments);return s!==i&&(r=(i=s)&&Zc(t,s,n)),r}return a._value=e,a}function tu(t,e,n){var r="style."+(t+="");if(arguments.length<2)return(r=this.tween(r))&&r._value;if(e==null)return this.tween(r,null);if(typeof e!="function")throw new Error;return this.tween(r,Qc(t,e,n??""))}function eu(t){return function(){this.textContent=t}}function nu(t){return function(){var e=t(this);this.textContent=e??""}}function ru(t){return this.tween("text",typeof t=="function"?nu(zn(this,"text",t)):eu(t==null?"":t+""))}function iu(t){return function(e){this.textContent=t.call(this,e)}}function au(t){var e,n;function r(){var i=t.apply(this,arguments);return i!==n&&(e=(n=i)&&iu(i)),e}return r._value=t,r}function su(t){var e="text";if(arguments.length<1)return(e=this.tween(e))&&e._value;if(t==null)return this.tween(e,null);if(typeof t!="function")throw new Error;return this.tween(e,au(t))}function ou(){for(var t=this._name,e=this._id,n=Ti(),r=this._groups,i=r.length,a=0;a<i;++a)for(var s=r[a],o=s.length,u,l=0;l<o;++l)if(u=s[l]){var c=X(u,e);Ne(u,t,n,l,s,{time:c.time+c.delay+c.duration,delay:0,duration:c.duration,ease:c.ease})}return new at(r,this._parents,t,n)}function lu(){var t,e,n=this,r=n._id,i=n.size();return new Promise(function(a,s){var o={value:s},u={value:function(){--i===0&&a()}};n.each(function(){var l=J(this,r),c=l.on;c!==t&&(e=(t=c).copy(),e._.cancel.push(o),e._.interrupt.push(o),e._.end.push(u)),l.on=e}),i===0&&a()})}var cu=0;function at(t,e,n,r){this._groups=t,this._parents=e,this._name=n,this._id=r}function Ti(){return++cu}var tt=Kt.prototype;at.prototype={constructor:at,select:Vc,selectAll:qc,selectChild:tt.selectChild,selectChildren:tt.selectChildren,filter:Fc,merge:$c,selection:Xc,transition:ou,call:tt.call,nodes:tt.nodes,node:tt.node,size:tt.size,empty:tt.empty,each:tt.each,on:Bc,attr:bc,attrTween:Sc,style:Jc,styleTween:tu,text:ru,textTween:su,remove:Hc,tween:dc,delay:Cc,duration:Mc,ease:zc,easeVarying:Ic,end:lu,[Symbol.iterator]:tt[Symbol.iterator]};function uu(t){return((t*=2)<=1?t*t*t:(t-=2)*t*t+2)/2}var fu={time:null,delay:0,duration:250,ease:uu};function hu(t,e){for(var n;!(n=t.__transition)||!(n=n[e]);)if(!(t=t.parentNode))throw new Error(`transition ${e} not found`);return n}function du(t){var e,n;t instanceof at?(e=t._id,t=t._name):(e=Ti(),(n=fu).time=Mn(),t=t==null?null:t+"");for(var r=this._groups,i=r.length,a=0;a<i;++a)for(var s=r[a],o=s.length,u,l=0;l<o;++l)(u=s[l])&&Ne(u,t,e,l,s,n||hu(u,e));return new at(r,this._parents,t,e)}Kt.prototype.interrupt=uc;Kt.prototype.transition=du;function mu(t){var e=0,n=t.children,r=n&&n.length;if(!r)e=1;else for(;--r>=0;)e+=n[r].value;t.value=e}function pu(){return this.eachAfter(mu)}function gu(t,e){let n=-1;for(const r of this)t.call(e,r,++n,this);return this}function yu(t,e){for(var n=this,r=[n],i,a,s=-1;n=r.pop();)if(t.call(e,n,++s,this),i=n.children)for(a=i.length-1;a>=0;--a)r.push(i[a]);return this}function vu(t,e){for(var n=this,r=[n],i=[],a,s,o,u=-1;n=r.pop();)if(i.push(n),a=n.children)for(s=0,o=a.length;s<o;++s)r.push(a[s]);for(;n=i.pop();)t.call(e,n,++u,this);return this}function xu(t,e){let n=-1;for(const r of this)if(t.call(e,r,++n,this))return r}function bu(t){return this.eachAfter(function(e){for(var n=+t(e.data)||0,r=e.children,i=r&&r.length;--i>=0;)n+=r[i].value;e.value=n})}function wu(t){return this.eachBefore(function(e){e.children&&e.children.sort(t)})}function _u(t){for(var e=this,n=Eu(e,t),r=[e];e!==n;)e=e.parent,r.push(e);for(var i=r.length;t!==n;)r.splice(i,0,t),t=t.parent;return r}function Eu(t,e){if(t===e)return t;var n=t.ancestors(),r=e.ancestors(),i=null;for(t=n.pop(),e=r.pop();t===e;)i=t,t=n.pop(),e=r.pop();return i}function ku(){for(var t=this,e=[t];t=t.parent;)e.push(t);return e}function Su(){return Array.from(this)}function Au(){var t=[];return this.eachBefore(function(e){e.children||t.push(e)}),t}function Lu(){var t=this,e=[];return t.each(function(n){n!==t&&e.push({source:n.parent,target:n})}),e}function*Cu(){var t=this,e,n=[t],r,i,a;do for(e=n.reverse(),n=[];t=e.pop();)if(yield t,r=t.children)for(i=0,a=r.length;i<a;++i)n.push(r[i]);while(n.length)}function kt(t,e){t instanceof Map?(t=[void 0,t],e===void 0&&(e=Mu)):e===void 0&&(e=Nu);for(var n=new Wt(t),r,i=[n],a,s,o,u;r=i.pop();)if((s=e(r.data))&&(u=(s=Array.from(s)).length))for(r.children=s,o=u-1;o>=0;--o)i.push(a=s[o]=new Wt(s[o])),a.parent=r,a.depth=r.depth+1;return n.eachBefore(zu)}function Tu(){return kt(this).eachBefore(Pu)}function Nu(t){return t.children}function Mu(t){return Array.isArray(t)?t[1]:null}function Pu(t){t.data.value!==void 0&&(t.value=t.data.value),t.data=t.data.data}function zu(t){var e=0;do t.height=e;while((t=t.parent)&&t.height<++e)}function Wt(t){this.data=t,this.depth=this.height=0,this.parent=null}Wt.prototype=kt.prototype={constructor:Wt,count:pu,each:gu,eachAfter:vu,eachBefore:yu,find:xu,sum:bu,sort:wu,path:_u,ancestors:ku,descendants:Su,leaves:Au,links:Lu,copy:Tu,[Symbol.iterator]:Cu};function Ou(t,e){return t.parent===e.parent?1:2}function Be(t){var e=t.children;return e?e[0]:t.t}function Ue(t){var e=t.children;return e?e[e.length-1]:t.t}function Iu(t,e,n){var r=n/(e.i-t.i);e.c-=r,e.s+=n,t.c+=r,e.z+=n,e.m+=n}function Fu(t){for(var e=0,n=0,r=t.children,i=r.length,a;--i>=0;)a=r[i],a.z+=e,a.m+=e,e+=a.s+(n+=a.c)}function $u(t,e,n){return t.a.parent===e.parent?t.a:n}function he(t,e){this._=t,this.parent=null,this.children=null,this.A=null,this.a=this,this.z=0,this.m=0,this.c=0,this.s=0,this.t=null,this.i=e}he.prototype=Object.create(Wt.prototype);function Ru(t){for(var e=new he(t,0),n,r=[e],i,a,s,o;n=r.pop();)if(a=n._.children)for(n.children=new Array(o=a.length),s=o-1;s>=0;--s)r.push(i=n.children[s]=new he(a[s],s)),i.parent=n;return(e.parent=new he(null,0)).children=[e],e}function kr(){var t=Ou,e=1,n=1,r=null;function i(l){var c=Ru(l);if(c.eachAfter(a),c.parent.m=-c.z,c.eachBefore(s),r)l.eachBefore(u);else{var f=l,h=l,m=l;l.eachBefore(function(_){_.x<f.x&&(f=_),_.x>h.x&&(h=_),_.depth>m.depth&&(m=_)});var v=f===h?1:t(f,h)/2,E=v-f.x,S=e/(h.x+v+E),y=n/(m.depth||1);l.eachBefore(function(_){_.x=(_.x+E)*S,_.y=_.depth*y})}return l}function a(l){var c=l.children,f=l.parent.children,h=l.i?f[l.i-1]:null;if(c){Fu(l);var m=(c[0].z+c[c.length-1].z)/2;h?(l.z=h.z+t(l._,h._),l.m=l.z-m):l.z=m}else h&&(l.z=h.z+t(l._,h._));l.parent.A=o(l,h,l.parent.A||f[0])}function s(l){l._.x=l.z+l.parent.m,l.m+=l.parent.m}function o(l,c,f){if(c){for(var h=l,m=l,v=c,E=h.parent.children[0],S=h.m,y=m.m,_=v.m,A=E.m,T;v=Ue(v),h=Be(h),v&&h;)E=Be(E),m=Ue(m),m.a=l,T=v.z+_-h.z-S+t(v._,h._),T>0&&(Iu($u(v,l,f),l,T),S+=T,y+=T),_+=v.m,S+=h.m,A+=E.m,y+=m.m;v&&!Ue(m)&&(m.t=v,m.m+=_-y),h&&!Be(E)&&(E.t=h,E.m+=S-A,f=l)}return f}function u(l){l.x*=e,l.y=l.depth*n}return i.separation=function(l){return arguments.length?(t=l,i):t},i.size=function(l){return arguments.length?(r=!1,e=+l[0],n=+l[1],i):r?null:[e,n]},i.nodeSize=function(l){return arguments.length?(r=!0,e=+l[0],n=+l[1],i):r?[e,n]:null},i}const ae=t=>()=>t;function Du(t,{sourceEvent:e,target:n,transform:r,dispatch:i}){Object.defineProperties(this,{type:{value:t,enumerable:!0,configurable:!0},sourceEvent:{value:e,enumerable:!0,configurable:!0},target:{value:n,enumerable:!0,configurable:!0},transform:{value:r,enumerable:!0,configurable:!0},_:{value:i}})}function et(t,e,n){this.k=t,this.x=e,this.y=n}et.prototype={constructor:et,scale:function(t){return t===1?this:new et(this.k*t,this.x,this.y)},translate:function(t,e){return t===0&e===0?this:new et(this.k,this.x+this.k*t,this.y+this.k*e)},apply:function(t){return[t[0]*this.k+this.x,t[1]*this.k+this.y]},applyX:function(t){return t*this.k+this.x},applyY:function(t){return t*this.k+this.y},invert:function(t){return[(t[0]-this.x)/this.k,(t[1]-this.y)/this.k]},invertX:function(t){return(t-this.x)/this.k},invertY:function(t){return(t-this.y)/this.k},rescaleX:function(t){return t.copy().domain(t.range().map(this.invertX,this).map(t.invert,t))},rescaleY:function(t){return t.copy().domain(t.range().map(this.invertY,this).map(t.invert,t))},toString:function(){return"translate("+this.x+","+this.y+") scale("+this.k+")"}};var Ct=new et(1,0,0);pn.prototype=et.prototype;function pn(t){for(;!t.__zoom;)if(!(t=t.parentNode))return Ct;return t.__zoom}function He(t){t.stopImmediatePropagation()}function It(t){t.preventDefault(),t.stopImmediatePropagation()}function Bu(t){return(!t.ctrlKey||t.type==="wheel")&&!t.button}function Uu(){var t=this;return t instanceof SVGElement?(t=t.ownerSVGElement||t,t.hasAttribute("viewBox")?(t=t.viewBox.baseVal,[[t.x,t.y],[t.x+t.width,t.y+t.height]]):[[0,0],[t.width.baseVal.value,t.height.baseVal.value]]):[[0,0],[t.clientWidth,t.clientHeight]]}function Sr(){return this.__zoom||Ct}function Hu(t){return-t.deltaY*(t.deltaMode===1?.05:t.deltaMode?1:.002)*(t.ctrlKey?10:1)}function Vu(){return navigator.maxTouchPoints||"ontouchstart"in this}function qu(t,e,n){var r=t.invertX(e[0][0])-n[0][0],i=t.invertX(e[1][0])-n[1][0],a=t.invertY(e[0][1])-n[0][1],s=t.invertY(e[1][1])-n[1][1];return t.translate(i>r?(r+i)/2:Math.min(0,r)||Math.max(0,i),s>a?(a+s)/2:Math.min(0,a)||Math.max(0,s))}function Yu(){var t=Bu,e=Uu,n=qu,r=Hu,i=Vu,a=[0,1/0],s=[[-1/0,-1/0],[1/0,1/0]],o=250,u=nc,l=Ln("start","zoom","end"),c,f,h,m=500,v=150,E=0,S=10;function y(d){d.property("__zoom",Sr).on("wheel.zoom",_t,{passive:!1}).on("mousedown.zoom",Zt).on("dblclick.zoom",Qt).filter(i).on("touchstart.zoom",Ni).on("touchmove.zoom",Mi).on("touchend.zoom touchcancel.zoom",Pi).style("-webkit-tap-highlight-color","rgba(0,0,0,0)")}y.transform=function(d,b,g,w){var k=d.selection?d.selection():d;k.property("__zoom",Sr),d!==k?P(d,b,g,w):k.interrupt().each(function(){O(this,arguments).event(w).start().zoom(null,typeof b=="function"?b.apply(this,arguments):b).end()})},y.scaleBy=function(d,b,g,w){y.scaleTo(d,function(){var k=this.__zoom.k,L=typeof b=="function"?b.apply(this,arguments):b;return k*L},g,w)},y.scaleTo=function(d,b,g,w){y.transform(d,function(){var k=e.apply(this,arguments),L=this.__zoom,C=g==null?T(k):typeof g=="function"?g.apply(this,arguments):g,N=L.invert(C),z=typeof b=="function"?b.apply(this,arguments):b;return n(A(_(L,z),C,N),k,s)},g,w)},y.translateBy=function(d,b,g,w){y.transform(d,function(){return n(this.__zoom.translate(typeof b=="function"?b.apply(this,arguments):b,typeof g=="function"?g.apply(this,arguments):g),e.apply(this,arguments),s)},null,w)},y.translateTo=function(d,b,g,w,k){y.transform(d,function(){var L=e.apply(this,arguments),C=this.__zoom,N=w==null?T(L):typeof w=="function"?w.apply(this,arguments):w;return n(Ct.translate(N[0],N[1]).scale(C.k).translate(typeof b=="function"?-b.apply(this,arguments):-b,typeof g=="function"?-g.apply(this,arguments):-g),L,s)},w,k)};function _(d,b){return b=Math.max(a[0],Math.min(a[1],b)),b===d.k?d:new et(b,d.x,d.y)}function A(d,b,g){var w=b[0]-g[0]*d.k,k=b[1]-g[1]*d.k;return w===d.x&&k===d.y?d:new et(d.k,w,k)}function T(d){return[(+d[0][0]+ +d[1][0])/2,(+d[0][1]+ +d[1][1])/2]}function P(d,b,g,w){d.on("start.zoom",function(){O(this,arguments).event(w).start()}).on("interrupt.zoom end.zoom",function(){O(this,arguments).event(w).end()}).tween("zoom",function(){var k=this,L=arguments,C=O(k,L).event(w),N=e.apply(k,L),z=g==null?T(N):typeof g=="function"?g.apply(k,L):g,W=Math.max(N[1][0]-N[0][0],N[1][1]-N[0][1]),I=k.__zoom,H=typeof b=="function"?b.apply(k,L):b,Z=u(I.invert(z).concat(W/I.k),H.invert(z).concat(W/H.k));return function(V){if(V===1)V=H;else{var Q=Z(V),Me=W/Q[2];V=new et(Me,z[0]-Q[0]*Me,z[1]-Q[1]*Me)}C.zoom(null,V)}})}function O(d,b,g){return!g&&d.__zooming||new U(d,b)}function U(d,b){this.that=d,this.args=b,this.active=0,this.sourceEvent=null,this.extent=e.apply(d,b),this.taps=0}U.prototype={event:function(d){return d&&(this.sourceEvent=d),this},start:function(){return++this.active===1&&(this.that.__zooming=this,this.emit("start")),this},zoom:function(d,b){return this.mouse&&d!=="mouse"&&(this.mouse[1]=b.invert(this.mouse[0])),this.touch0&&d!=="touch"&&(this.touch0[1]=b.invert(this.touch0[0])),this.touch1&&d!=="touch"&&(this.touch1[1]=b.invert(this.touch1[0])),this.that.__zoom=b,this.emit("zoom"),this},end:function(){return--this.active===0&&(delete this.that.__zooming,this.emit("end")),this},emit:function(d){var b=ct(this.that).datum();l.call(d,this.that,new Du(d,{sourceEvent:this.sourceEvent,target:y,transform:this.that.__zoom,dispatch:l}),b)}};function _t(d,...b){if(!t.apply(this,arguments))return;var g=O(this,b).event(d),w=this.__zoom,k=Math.max(a[0],Math.min(a[1],w.k*Math.pow(2,r.apply(this,arguments)))),L=dt(d);if(g.wheel)(g.mouse[0][0]!==L[0]||g.mouse[0][1]!==L[1])&&(g.mouse[1]=w.invert(g.mouse[0]=L)),clearTimeout(g.wheel);else{if(w.k===k)return;g.mouse=[L,w.invert(L)],fe(this),g.start()}It(d),g.wheel=setTimeout(C,v),g.zoom("mouse",n(A(_(w,k),g.mouse[0],g.mouse[1]),g.extent,s));function C(){g.wheel=null,g.end()}}function Zt(d,...b){if(h||!t.apply(this,arguments))return;var g=d.currentTarget,w=O(this,b,!0).event(d),k=ct(d.view).on("mousemove.zoom",z,!0).on("mouseup.zoom",W,!0),L=dt(d,g),C=d.clientX,N=d.clientY;Tl(d.view),He(d),w.mouse=[L,this.__zoom.invert(L)],fe(this),w.start();function z(I){if(It(I),!w.moved){var H=I.clientX-C,Z=I.clientY-N;w.moved=H*H+Z*Z>E}w.event(I).zoom("mouse",n(A(w.that.__zoom,w.mouse[0]=dt(I,g),w.mouse[1]),w.extent,s))}function W(I){k.on("mousemove.zoom mouseup.zoom",null),Nl(I.view,w.moved),It(I),w.event(I).end()}}function Qt(d,...b){if(t.apply(this,arguments)){var g=this.__zoom,w=dt(d.changedTouches?d.changedTouches[0]:d,this),k=g.invert(w),L=g.k*(d.shiftKey?.5:2),C=n(A(_(g,L),w,k),e.apply(this,b),s);It(d),o>0?ct(this).transition().duration(o).call(P,C,w,d):ct(this).call(y.transform,C,w,d)}}function Ni(d,...b){if(t.apply(this,arguments)){var g=d.touches,w=g.length,k=O(this,b,d.changedTouches.length===w).event(d),L,C,N,z;for(He(d),C=0;C<w;++C)N=g[C],z=dt(N,this),z=[z,this.__zoom.invert(z),N.identifier],k.touch0?!k.touch1&&k.touch0[2]!==z[2]&&(k.touch1=z,k.taps=0):(k.touch0=z,L=!0,k.taps=1+!!c);c&&(c=clearTimeout(c)),L&&(k.taps<2&&(f=z[0],c=setTimeout(function(){c=null},m)),fe(this),k.start())}}function Mi(d,...b){if(this.__zooming){var g=O(this,b).event(d),w=d.changedTouches,k=w.length,L,C,N,z;for(It(d),L=0;L<k;++L)C=w[L],N=dt(C,this),g.touch0&&g.touch0[2]===C.identifier?g.touch0[0]=N:g.touch1&&g.touch1[2]===C.identifier&&(g.touch1[0]=N);if(C=g.that.__zoom,g.touch1){var W=g.touch0[0],I=g.touch0[1],H=g.touch1[0],Z=g.touch1[1],V=(V=H[0]-W[0])*V+(V=H[1]-W[1])*V,Q=(Q=Z[0]-I[0])*Q+(Q=Z[1]-I[1])*Q;C=_(C,Math.sqrt(V/Q)),N=[(W[0]+H[0])/2,(W[1]+H[1])/2],z=[(I[0]+Z[0])/2,(I[1]+Z[1])/2]}else if(g.touch0)N=g.touch0[0],z=g.touch0[1];else return;g.zoom("touch",n(A(C,N,z),g.extent,s))}}function Pi(d,...b){if(this.__zooming){var g=O(this,b).event(d),w=d.changedTouches,k=w.length,L,C;for(He(d),h&&clearTimeout(h),h=setTimeout(function(){h=null},m),L=0;L<k;++L)C=w[L],g.touch0&&g.touch0[2]===C.identifier?delete g.touch0:g.touch1&&g.touch1[2]===C.identifier&&delete g.touch1;if(g.touch1&&!g.touch0&&(g.touch0=g.touch1,delete g.touch1),g.touch0)g.touch0[1]=this.__zoom.invert(g.touch0[0]);else if(g.end(),g.taps===2&&(C=dt(C,this),Math.hypot(f[0]-C[0],f[1]-C[1])<S)){var N=ct(this).on("dblclick.zoom");N&&N.apply(this,arguments)}}}return y.wheelDelta=function(d){return arguments.length?(r=typeof d=="function"?d:ae(+d),y):r},y.filter=function(d){return arguments.length?(t=typeof d=="function"?d:ae(!!d),y):t},y.touchable=function(d){return arguments.length?(i=typeof d=="function"?d:ae(!!d),y):i},y.extent=function(d){return arguments.length?(e=typeof d=="function"?d:ae([[+d[0][0],+d[0][1]],[+d[1][0],+d[1][1]]]),y):e},y.scaleExtent=function(d){return arguments.length?(a[0]=+d[0],a[1]=+d[1],y):[a[0],a[1]]},y.translateExtent=function(d){return arguments.length?(s[0][0]=+d[0][0],s[1][0]=+d[1][0],s[0][1]=+d[0][1],s[1][1]=+d[1][1],y):[[s[0][0],s[0][1]],[s[1][0],s[1][1]]]},y.constrain=function(d){return arguments.length?(n=d,y):n},y.duration=function(d){return arguments.length?(o=+d,y):o},y.interpolate=function(d){return arguments.length?(u=d,y):u},y.on=function(){var d=l.on.apply(l,arguments);return d===l?y:d},y.clickDistance=function(d){return arguments.length?(E=(d=+d)*d,y):Math.sqrt(E)},y.tapDistance=function(d){return arguments.length?(S=+d,y):S},y}function Xu(t,e){let n;return(...r)=>{n||(t(...r),n=!0,setTimeout(()=>n=!1,e))}}class Wu{constructor(e){this.container=e,this.svg=null,this.g=null,this.width=e.clientWidth||1e3,this.height=Math.max(window.innerHeight-200,600),this.expandedNodes=new Set,this.zoomLevel=1,this.nodeId=0,this.zoomBehavior=null,this.treeData=null,this.listeners=new Map,this.renderControls()}renderControls(){const e=document.createElement("div");e.className="diagram-controls",e.innerHTML=`
