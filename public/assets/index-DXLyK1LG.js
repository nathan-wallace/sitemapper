(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const t of n)if(t.type==="childList")for(const d of t.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&s(d)}).observe(document,{childList:!0,subtree:!0});function i(n){const t={};return n.integrity&&(t.integrity=n.integrity),n.referrerPolicy&&(t.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?t.credentials="include":n.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function s(n){if(n.ep)return;n.ep=!0;const t=i(n);fetch(n.href,t)}})();function F(){const r=document.createElement("header"),i=window.location.pathname==="/results";return r.innerHTML=`
  ...
  `,r}function U(r){const e=document.createElement("section");e.innerHTML=`
    <h2>Load Sitemap</h2>
...
`)],{type:"text/csv"}),p=document.createElement("a");p.href=URL.createObjectURL(u),p.download="sitemap.csv",p.click()}))}class P{constructor(e){this.container=e,this.isExpanded=!0}render(e,i={},s="url"){if(showLoading(50),this.container.innerHTML="",!e||e.length===0){this.container.textContent="No URLs found in sitemap.",hideLoading();return}const n=e.filter(({priority:a,lastmod:o})=>{const l=parseFloat(i.priority||0),c=i.lastmod?new Date(i.lastmod):null;return(a==="N/A"||parseFloat(a)>=l)&&(!c||o!=="N/A"&&new Date(o)>=c)}),t=T(n),d=document.createElement("ul");Object.keys(t).sort((a,o)=>a.localeCompare(o)).forEach(a=>{d.appendChild(w(a,t[a],s,this.isExpanded))}),this.container.appendChild(d),hideLoading()}toggleExpansion(){const e=this.container.querySelectorAll(".toggle");this.isExpanded=!this.isExpanded,e.forEach(i=>{const s=i.nextElementSibling;s&&s.tagName==="UL"&&(s.classList.toggle("hidden",!this.isExpanded),i.classList.toggle("open",this.isExpanded),i.setAttribute("aria-expanded",this.isExpanded))})}}function T(r){const e={};return r.forEach(({loc:i,lastmod:s,changefreq:n,priority:t})=>{const a=`${new URL(i).origin}/`;e[a]||(e[a]={children:{},details:{}});let o=e[a];const l=i.replace(a,"").split("/").filter(Boolean);l.length===0?o.details={lastmod:s,changefreq:n,priority:t}:l.forEach((c,u)=>{const p=a+l.slice(0,u+1).join("/");o.children[p]||(o.children[p]={children:{},details:{}}),u===l.length-1&&(o.children[p].details={lastmod:s,changefreq:n,priority:t}),o=o.children[p]})}),e}function w(r,e,i,s){const n=document.createElement("li"),t=document.createElement("span");t.textContent=r;const d=Object.keys(e.children);if(d.length>0){t.className="toggle",t.setAttribute("aria-expanded",s?"true":"false"),s&&t.classList.add("open");const a=document.createElement("ul");d.sort((o,l)=>o.localeCompare(l)).forEach(o=>{a.appendChild(w(o,e.children[o],i,s))}),n.appendChild(t),n.appendChild(a)}else n.appendChild(t);if(e.details.lastmod&&e.details.lastmod!=="N/A"){const a=document.createElement("span");a.className="details",a.textContent=` (Priority: ${e.details.priority}, Last Modified: ${e.details.lastmod}, ${e.details.changefreq})`,n.appendChild(a)}return n}function E(){const r=document.createElement("filters"),i=window.location.pathname==="/results";return r.innerHTML=`
  ...
  `,r.appendChild(e);const i=e.querySelector("#filters"),s=e.querySelector("#toggleFiltersBtn");s==null||s.addEventListener("click",()=>{const t=s.getAttribute("aria-expanded")==="true";s.setAttribute("aria-expanded",String(!t)),i==null||i.classList.toggle("hidden")});const n=new URLSearchParams(window.location.search).get("id");if(!n){f("Missing ID in URL");return}R(),fetch(`/results-data?id=${n}`).then(t=>{if(!t.ok)throw new Error("Failed to load results data.");return t.json()}).then(t=>{if(g(),!Array.isArray(t.urls)||t.urls.length===0){f("No results found.");return}h=t.urls;const d=e.querySelector("#treeView");L=new P(d),L.render(h),k({urls:h,treeContainer:d,filtersContainer:i,id:n}),E(),e.querySelector("#stats").innerHTML=`<p>Total URLs: ${h.length}</p>`}).catch(t=>{g(),f(`Error loading data: ${t.message}`)})}document.getElementById("app");const v=document.querySelector("header"),m=document.getElementById("view");function b(){v.innerHTML="",v.appendChild(F())}function y(r){switch(m.innerHTML="",r){case"/":U(m);break;case"/results":O(m);break;default:m.innerHTML="<h2>404 - Page not found</h2>"}}function $(r){const e=r.target.closest("a");if(e&&e.href.startsWith(window.location.origin)){r.preventDefault();const i=new URL(e.href).pathname;window.history.pushState({},"",i),b(),y(i)}}function j(){b(),y(window.location.pathname),document.body.addEventListener("click",$),window.addEventListener("popstate",()=>{b(),y(window.location.pathname)})}j();
